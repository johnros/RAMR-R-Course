...
```{r}
auto[,list(Price=mean(price, na.rm=TRUE), Power=mean(powerPS)), by=vehicleType]
```

You can use `.()` to replace the longer `list()` command:
```{r}
auto[,.(Price=mean(price, na.rm=TRUE), Power=mean(powerPS)), by=vehicleType]
```

And split by multiple variables:
```{r}
table1 <- auto[,.(Price=mean(price, na.rm=TRUE), Power=mean(powerPS)), by=.(vehicleType,fuelType)] %>% head
```
...
auto[,.(Power=mean(price, na.rm=TRUE)), by=.(PriceRange=price>1e4)] 
```
